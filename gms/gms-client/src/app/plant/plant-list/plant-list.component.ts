...
@Component({
  ...
  <div class="plant-page">
    ...
    <div class="plant-page__filter-container">
      ...
      <input type="button" (click)="filterPlants()" value="Filter Plants" class="plant-page__filter-button">
    </div>
    ...
    @if (serverMessage) {
      <div [ngClass]="{'message-alert': serverMessageType === 'error', 'message-success': serverMessageType === 'success'}">{{ serverMessage }}</div>
    }
    ...
    <table *ngIf="plants && plants.length > 0" class="plant-page__table">
      ...
      <tbody class="plant-page__table-body">
        <tr *ngFor="let plant of plants" class="plant-page__table-row" [appHighlightRecent]='plant.datePlanted ?? ""'>
    ...
    <p *ngIf="!plants || plants.length === 0" class="plant-page__no-plants">No plants found, consider adding one...</p>
  </div>
  `,
  styles: `
    .plant-page {
